class stsCalendar {
  ...
  render() {
    ...
    for (let m = 0; m < this.monthsToShow; m++) {
      ...
      // Find the index of the actualStartDayIndex in daysOfWeek:
  ...
  handleDateClick(dayElement, dateStr) {
    ...
    if (this.multipleSelection && !this.RandomSelection) {
      if (!this.startDate || (this.startDate && this.endDate)) {
        ...
        if (this.isLockedDateInRange(rangeStart, rangeEnd)) {
          this.resetWithError("Has booked dates in range.");
          return;
        }
    ...
    // Optional: could dispatch a custom event here if you want to notify outside
    // this.element.dispatchEvent(new CustomEvent('datesChanged', { detail: this.getSelectedDates() }));
  }
